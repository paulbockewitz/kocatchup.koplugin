...
describe("kocatchup_cache last-read tracking", function()
    before_each(function() H.reset() end)

    it("round-trips the timestamp and flushes (load-bearing at close time)", function()
        local ds = H.make_doc_settings()
        Cache.touch_last_read(ds, 1723300000)
        assert.truthy(ds.flushed > 0, "touch must flush explicitly")
        assert.are.equal(1723300000, Cache.last_read(ds))
    end)

    it("reads nil for absent or corrupt values", function()
        local ds = H.make_doc_settings()
        assert.is_nil(Cache.last_read(ds))
        ds:saveSetting(Cache.LAST_READ_KEY, "not a number")
        assert.is_nil(Cache.last_read(ds))
        assert.is_nil(Cache.last_read(nil))
    end)
end)
